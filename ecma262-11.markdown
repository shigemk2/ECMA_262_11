...
with 文は、現在の実行コンテキストのレキシカル環境に生成したオブジェクトについてオブジェクト型環境レコードを追加します。 その場合、引数としたレキシカル環境を使って文を実行します。 最後にそれは元のレキシカル環境を復元します。
