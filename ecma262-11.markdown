...
2. Rは、CaseBlockを評価した結果とし、引数として GetValue(exprRef)を渡す。
3. R.type が、 breakであり、 R.target が、現在のラベルのセットの中にある場合、return (normal, R.value, empty)
4. Rを返す
...
1はexprRefの定義
2はRの定義(RはたぶんReturn)
3 Rはtypeとtargetとvalueというキーを持つオブジェクト?
